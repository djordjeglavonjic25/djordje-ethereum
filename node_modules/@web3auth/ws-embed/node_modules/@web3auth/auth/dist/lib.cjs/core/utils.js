...
function getHashQueryParams(replaceUrl = false) {
  ...
  if (queryResult) {
    try {
      const queryParams = JSON.parse(metadataHelpers.decodeBase64Url(queryResult));
  ...
  if (hashResult) {
    try {
      const hashParams = JSON.parse(metadataHelpers.decodeBase64Url(hashResult));
...
exports.isAuthFlowError = isAuthFlowError;
