...
class WsEmbed {
  constructor({
    ...
    _defineProperty(this, "enableLegacyConfig", false);
  ...
  async init({
    ...
    this.enableLegacyConfig = enableLegacyConfig;
  ...
  async login(params = {}) {
    ...
    try {
      ...
      const requestParams = params.authConnection ? [params.authConnection, params.login_hint, params.authConnectionId, params.groupedAuthConnectionId, windowId] : [];
      // If user is already logged in, we assume they have given access to the website
      const res = await this.provider.rpcRequest({
        id: baseControllers.randomId(),
        method,
  ...
  async connectWithSession(params) {
    if (!this.isInitialized) throw new Error("Call init() first");
    if (!params.sessionId) throw new Error("sessionId is required");
    if (!this.communicationProvider.hasAccessTokenProvider) throw new Error("accessTokenProvider is required. Call setAccessTokenProvider() first");
    try {
      const res = await this.communicationProvider.request({
        ...
        params: [`${params.sessionId}:${params.sessionNamespace || ""}`, params.idToken]
      });
      loglevel.info("check: res", res);
      if (res.success) {
  ...
  setAccessTokenProvider(provider) {
    if (!this.isInitialized) throw new Error("Call init() first");
    this.communicationProvider.setAccessTokenProvider(provider);
  }
  async logout() {
  ...
  async setupWeb3(providerParams) {
    ...
    const providerStream = new auth.PostMessageStream({
      name: "embed_torus",
      target: "iframe_torus",
    ...
    const communicationStream = new auth.PostMessageStream({
    ...
    const communicationProvider$1 = new communicationProvider.default(communicationStream, {}, {
      buttonPosition: this.buttonPosition
    });
    inPageProvider$1.tryWindowHandle = async payload => {
      ...
      const result = await inPageProvider$1.rpcEngine.handle(_payload);
      return result;
    };
    communicationProvider$1.tryWindowHandle = async payload => {
      const _payload = payload;
      const confirmationStrategyFinal = this.getConfirmationStrategyFinal(payload);
      const {
        allowWalletService
      ...
      const result = await communicationProvider$1.rpcEngine.handle(_payload);
      return result;
    };
