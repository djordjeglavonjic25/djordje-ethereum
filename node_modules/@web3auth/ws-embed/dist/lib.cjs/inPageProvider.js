...
class TorusInPageProvider extends baseProvider {
  ...
  async rpcRequest(request, isInternal = false) {
    ...
    if (Array.isArray(result) && [constants.EVM_METHOD_TYPES.GET_ACCOUNTS, constants.EVM_METHOD_TYPES.ETH_REQUEST_ACCOUNTS, constants.SOLANA_METHOD_TYPES.SOLANA_REQUEST_ACCOUNTS, constants.SOLANA_METHOD_TYPES.GET_ACCOUNTS].includes(request.method)) {
      // handle accounts changing
      const method = request.method;
      // check if the accounts request matches the current chain namespace
      const isEvmMethod = method === constants.EVM_METHOD_TYPES.GET_ACCOUNTS || method === constants.EVM_METHOD_TYPES.ETH_REQUEST_ACCOUNTS;
      const isSolanaMethod = method === constants.SOLANA_METHOD_TYPES.GET_ACCOUNTS || method === constants.SOLANA_METHOD_TYPES.SOLANA_REQUEST_ACCOUNTS;
      const isSolanaChain = this.chainId != null && Object.values(constants.SOLANA_CHAIN_IDS).includes(this.chainId);
      const isSelectedAddressEvm = embedUtils.isEvmAddress(this.selectedAddress);
      const isSelectedAddressSolana = embedUtils.isSolanaAddress(this.selectedAddress);
      ...
      if (accountsRequestMatchesCurrentChain) {
        this.handleAccountsChanged(result, method === constants.EVM_METHOD_TYPES.GET_ACCOUNTS, isInternal);
      }
    }
    return result;
  }
