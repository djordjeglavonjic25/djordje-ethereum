...
import { THEME_MODES, PostMessageStream, serializeError } from '@web3auth/auth';
...
class WsEmbed {
  ...
  async setupWeb3(providerParams) {
    ...
    const providerStream = new PostMessageStream({
      name: "embed_torus",
      target: "iframe_torus",
    ...
    inPageProvider.tryWindowHandle = async payload => {
      ...
      const result = await inPageProvider.rpcEngine.handle(_payload);
      return result;
    };
    communicationProvider.tryWindowHandle = async payload => {
      const _payload = payload;
      const confirmationStrategyFinal = this.getConfirmationStrategyFinal(payload);
      const {
        allowWalletService
      ...
      const result = await communicationProvider.rpcEngine.handle(_payload);
      return result;
    };
