...
import { EVM_METHOD_TYPES, SOLANA_METHOD_TYPES, SOLANA_CHAIN_IDS } from './constants.js';
import { isEvmAddress, isSolanaAddress } from './embedUtils.js';
...
class TorusInPageProvider extends BaseProvider {
   ...
   */
  async rpcRequest(request, isInternal = false) {
    if (!request.jsonrpc) {
      request.jsonrpc = "2.0";
    }
    if (request.method === "wallet_getProviderState") {
      const providerState = await this.rpcEngine.handle(request);
      return providerState;
    }
    const result = await this.tryWindowHandle(request);
    if (Array.isArray(result) && [EVM_METHOD_TYPES.GET_ACCOUNTS, EVM_METHOD_TYPES.ETH_REQUEST_ACCOUNTS, SOLANA_METHOD_TYPES.SOLANA_REQUEST_ACCOUNTS, SOLANA_METHOD_TYPES.GET_ACCOUNTS].includes(request.method)) {
      // handle accounts changing
      const method = request.method;

      // check if the accounts request matches the current chain namespace
      const isEvmMethod = method === EVM_METHOD_TYPES.GET_ACCOUNTS || method === EVM_METHOD_TYPES.ETH_REQUEST_ACCOUNTS;
      const isSolanaMethod = method === SOLANA_METHOD_TYPES.GET_ACCOUNTS || method === SOLANA_METHOD_TYPES.SOLANA_REQUEST_ACCOUNTS;
      const isSolanaChain = this.chainId != null && Object.values(SOLANA_CHAIN_IDS).includes(this.chainId);
      const isSelectedAddressEvm = isEvmAddress(this.selectedAddress);
      const isSelectedAddressSolana = isSolanaAddress(this.selectedAddress);
      const accountsRequestMatchesCurrentChain = isEvmMethod && isSelectedAddressEvm && !isSolanaChain || isSolanaChain && isSolanaMethod && isSelectedAddressSolana;
      if (accountsRequestMatchesCurrentChain) {
        this.handleAccountsChanged(result, method === EVM_METHOD_TYPES.GET_ACCOUNTS, isInternal);
      }
    }
    return result;
  }
