...
import { keccak256Bytes, utf8ToBytes, encryptData, bytesToHex, decryptData } from '@toruslabs/metadata-helpers';
...
class SecurePubSub {
  ...
  async publish(topic, message) {
    ...
    const topicPrivKey = keccak256Bytes(utf8ToBytes(topic));
    const encryptedData = await encryptData(topicPrivKey, message);
    const signature = await sign(topicPrivKey, keccak256Bytes(utf8ToBytes(encryptedData)));
    const fetchBody = {
      key: bytesToHex(getPublic(topicPrivKey)),
      // already padded
      data: encryptedData,
  ...
  async subscribe(topic) {
    ...
    const topicPrivKey = keccak256Bytes(utf8ToBytes(topic));
    const topicPubKey = bytesToHex(getPublic(topicPrivKey)); // already padded
    ...
    const returnPromise = new Promise((resolve, reject) => {
      const listener = async ev => {
        try {
          const decData = await decryptData(topicPrivKey, ev);
