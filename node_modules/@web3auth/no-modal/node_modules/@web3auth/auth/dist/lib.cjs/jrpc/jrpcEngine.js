...
function constructFallbackError(error) {
  const {
    message = "",
    code = errorConstants.errorCodes.rpc.internal,
    stack = "Stack trace is not available.",
    data = ""
  } = error;
  const codeNumber = utils.isValidNumber(code) ? parseInt(code.toString()) : errorConstants.errorCodes.rpc.internal;
  return {
...
class JRPCEngine extends safeEventEmitter.SafeEventEmitter {
  ...
  static _checkForCompletion(_req, res, isComplete) {
    if (!jrpc.isJRPCSuccess(res) && !jrpc.isJRPCFailure(res)) {
    ...
    try {
      if (reqs.length === 0) {
        const error = new serializableError.SerializableError({
          code: errorConstants.errorCodes.rpc.invalidRequest,
      ...
      reqs.map(this._promiseHandle.bind(this)))).filter(response => response !== undefined);
  ...
  async _handle(callerReq, cb) {
    ...
    if (!jrpc.isValidMethod(callerReq)) {
      const error = new serializableError.SerializableError({
        code: errorConstants.errorCodes.rpc.invalidRequest,
