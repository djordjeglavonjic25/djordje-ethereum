...
var v2utils = require('./v2utils.js');
...
class JRPCEngineV2 {
  ...
  static create({
    ...
    if (middleware.length === 0) {
      throw new v2utils.JsonRpcEngineError("Middleware array cannot be empty");
    }
    const mw = middleware;
  ...
  async handle(request, {
    ...
    const isReq = jrpc.isRequest(request);
    const {
      result
    } = await this._handle(request, context);
    if (isReq && result === undefined) {
      throw new v2utils.JsonRpcEngineError(`Nothing ended request: ${jrpc.stringify(request)}`);
    }
    return result;
  }
  /**
   ...
   */
  async _handle(originalRequest, rawContext = new MiddlewareContext.MiddlewareContext()) {
    this._assertIsNotDestroyed();
    ...
    const context = MiddlewareContext.MiddlewareContext.isInstance(rawContext) ? rawContext : new MiddlewareContext.MiddlewareContext(rawContext);
  ...
  _makeNextFactory(middlewareIterator, state, context) {
    const makeNext = () => {
      ...
      const next = async (request = state.request) => {
        if (wasCalled) {
          throw new v2utils.JsonRpcEngineError(`Middleware attempted to call next() multiple times for request: ${jrpc.stringify(request)}`);
        }
        wasCalled = true;
  ...
  _updateResult(result, state) {
    if (jrpc.isNotification(state.request) && result !== undefined) {
      throw new v2utils.JsonRpcEngineError(`Result returned for notification: ${jrpc.stringify(state.request)}`);
    }
  ...
  _assertValidNextRequest(currentRequest, nextRequest) {
    if (nextRequest.jsonrpc !== currentRequest.jsonrpc) {
      throw new v2utils.JsonRpcEngineError(`Middleware attempted to modify readonly property "jsonrpc" for request: ${jrpc.stringify(currentRequest)}`);
    }
    if (utils.hasProperty(nextRequest, "id") !== utils.hasProperty(currentRequest, "id") ||
    ...
    nextRequest.id !== currentRequest.id) {
      throw new v2utils.JsonRpcEngineError(`Middleware attempted to modify readonly property "id" for request: ${jrpc.stringify(currentRequest)}`);
    }
  }
  _assertIsNotDestroyed() {
    if (_classPrivateFieldGet(_isDestroyed, this)) {
      throw new v2utils.JsonRpcEngineError("Engine is destroyed");
    }
  }
}

exports.JRPCEngineV2 = JRPCEngineV2;
