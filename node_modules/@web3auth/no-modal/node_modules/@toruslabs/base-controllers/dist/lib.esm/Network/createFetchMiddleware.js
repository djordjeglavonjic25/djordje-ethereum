...
import { serializeError, rpcErrors } from '@web3auth/auth';
...
function createFetchMiddleware({
  ...
  return async ({
    ...
    const isSkippedAnalytics = !Object.values(ANALYTICS_TRACKED_NETWORK_JPRC_METHODS).includes(request.method);
    const trackData = {
      chain_id: getCaipChainId(providerConfig),
      chain_rpc_target: getHostname(fetchUrl),
      method: request.method
    };
    if (!isSkippedAnalytics && transactionDecoder) {
      const [sender, txHash] = await transactionDecoder(request, (providerConfig === null || providerConfig === void 0 ? void 0 : providerConfig.chainId) || "");
      trackData.sender = sender;
      trackData.transaction_hash = txHash;
    }
    let errorAfterMaxAttempts;
    ...
    // after the maximum retries, re-throw the error from the last attempt
    if (errorAfterMaxAttempts) {
      const serializedErr = await serializeError(errorAfterMaxAttempts);
      throw serializedErr;
    }
  };
}
