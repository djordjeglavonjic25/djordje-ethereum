...
class SecurePubSub {
  ...
  async publish(topic, message) {
    ...
    const topicPrivKey = metadataHelpers.keccak256Bytes(metadataHelpers.utf8ToBytes(topic));
    const encryptedData = await metadataHelpers.encryptData(topicPrivKey, message);
    const signature = await eccrypto.sign(topicPrivKey, metadataHelpers.keccak256Bytes(metadataHelpers.utf8ToBytes(encryptedData)));
    const fetchBody = {
      key: metadataHelpers.bytesToHex(eccrypto.getPublic(topicPrivKey)),
      // already padded
      data: encryptedData,
      signature: metadataHelpers.bytesToHex(signature),
  ...
  async subscribe(topic) {
    ...
    const topicPrivKey = metadataHelpers.keccak256Bytes(metadataHelpers.utf8ToBytes(topic));
    const topicPubKey = metadataHelpers.bytesToHex(eccrypto.getPublic(topicPrivKey)); // already padded
    ...
    const returnPromise = new Promise((resolve, reject) => {
      const listener = async ev => {
        try {
          const decData = await metadataHelpers.decryptData(topicPrivKey, ev);
