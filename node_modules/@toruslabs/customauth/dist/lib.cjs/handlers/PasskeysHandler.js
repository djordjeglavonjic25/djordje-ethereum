...
class PasskeysHandler extends AbstractLoginHandler {
  ...
  async getUserInfo(parameters) {
    ...
    const {
      sessionId
    } = JSON.parse(metadataHelpers.decodeBase64Url(extraParams)) || {};
    if (!sessionId) {
      throw new Error("sessionId not found");
    }
    if (!metadataHelpers.isHexString(sessionId)) {
    ...
    const {
      ...
      transports,
      username
    } = await sessionHelper.fetchDataFromBroadcastServer(sessionId, this.params.storageServerUrl);
