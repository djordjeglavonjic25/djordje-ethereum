...
const handleRedirectParameters = (hash, queryParameters) => {
  ...
  if (Object.keys(hashParameters).length > 0 && hashParameters.state) {
    instanceParameters = JSON.parse(metadataHelpers.decodeBase64Url(decodeURIComponent(decodeURIComponent(hashParameters.state)))) || {};
    error = hashParameters.error_description || hashParameters.error || error;
  } else if (Object.keys(queryParameters).length > 0 && queryParameters.state) {
    instanceParameters = JSON.parse(metadataHelpers.decodeBase64Url(decodeURIComponent(decodeURIComponent(queryParameters.state)))) || {};
...
const objectToAuthDataMap = tgAuthenticationResult => {
  return JSON.parse(metadataHelpers.decodeBase64Url(tgAuthenticationResult));
};
...
function decodeToken(token) {
  ...
  return {
    header: JSON.parse(metadataHelpers.decodeBase64Url(header)),
    payload: JSON.parse(metadataHelpers.decodeBase64Url(payload))
  };
}
