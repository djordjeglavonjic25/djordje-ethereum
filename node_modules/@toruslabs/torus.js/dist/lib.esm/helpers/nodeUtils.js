...
import { bytesToHex, generatePrivateKey, thresholdSame, derivePubKey, bigintToHex, toBigIntBE, mod, generateAddressFromPrivKey, generateAddressFromPubKey, getSecpKeyFromEd25519, calculateMedian, keccak256, utf8ToBytes, hexToBytes, bytesToNumberBE, kCombinations, lagrangeInterpolation } from '@toruslabs/metadata-helpers';
import { config } from '../config.js';
import { JRPC_METHODS } from '../constants.js';
import log from '../loglevel.js';
import { Some } from '../some.js';
import { callAllowApi, CitadelAllowParamsSetOrUnsetFlag } from './citadelUtils.js';
import { getProxyCoordinatorEndpointIndex, normalizeKeysResult, normalizeLookUpResult, retryCommitment, base64ToBytes, bytesToBase64 } from './common.js';
import { generateShares, isV2NonceResult } from './keyUtils.js';
    ...
    if (keyResult && !nonceResult && !extendedVerifierId && !isLegacyNetwork(network)) {
      for (let i = 0; i < lookupResults.length; i++) {
        ...
        if (x1 && !x1.error) {
          ...
          const pubNonceX = nonceData && isV2NonceResult(nonceData) ? (_nonceData$pubNonce = nonceData.pubNonce) === null || _nonceData$pubNonce === void 0 ? void 0 : _nonceData$pubNonce.x : undefined;
  ...
  return Some(promiseArrRequest, async (shareResponseResult, sharedState) => {
    ...
    shareResponses.forEach(x => {
      ...
      if (requiredShareResponse && !thresholdNonceData && !verifierParams.extended_verifier_id) {
        ...
        const pubNonce = nonceData && isV2NonceResult(nonceData) ? (_nonceData$pubNonce2 = nonceData.pubNonce) === null || _nonceData$pubNonce2 === void 0 ? void 0 : _nonceData$pubNonce2.x : undefined;
      ...
      for (let i = 0; i < completedRequests.length; i += 1) {
        ...
        if ((keys === null || keys === void 0 ? void 0 : keys.length) > 0) {
          ...
          if (latestKey.share_metadata) {
            sharePromises.push(decryptNodeDataWithPadding(latestKey.share_metadata, new TextDecoder().decode(base64ToBytes(latestKey.share)), sessionAuthKey).catch(err => log.error("share decryption", err)));
          }
        } else {
    ...
    let metadataNonce = (_nonceResult = nonceResult) !== null && _nonceResult !== void 0 && _nonceResult.nonce ? toBigIntBE(nonceResult.nonce) : 0n;
    let finalPubKey;
    let pubNonce;
    let typeOfUser = "v1";
