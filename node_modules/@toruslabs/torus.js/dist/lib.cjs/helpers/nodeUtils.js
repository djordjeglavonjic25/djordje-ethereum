...
var citadelUtils = require('./citadelUtils.js');
...
const GetPubKeyOrKeyAssign = async params => {
  ...
  const result = await some.Some(lookupPromises, async lookupResults => {
    ...
    const errorResult = metadataHelpers.thresholdSame(lookupResults.map(x2 => x2 && x2.error), minThreshold);
    const keyResult = metadataHelpers.thresholdSame(lookupPubKeys.map(x3 => x3 && common.normalizeKeysResult(x3.result)), minThreshold);
    // check for nonce result in response if not a extendedVerifierId and not a legacy network
    if (keyResult && !nonceResult && !extendedVerifierId && !networkUtils.isLegacyNetwork(network)) {
      for (let i = 0; i < lookupResults.length; i++) {
        const x1 = lookupResults[i];
        if (x1 && !x1.error) {
          var _x1$result, _nonceData$pubNonce;
          ...
          const nonceData = (_x1$result = x1.result) === null || _x1$result === void 0 ? void 0 : _x1$result.keys[0].nonce_data;
          const pubNonceX = nonceData && keyUtils.isV2NonceResult(nonceData) ? (_nonceData$pubNonce = nonceData.pubNonce) === null || _nonceData$pubNonce === void 0 ? void 0 : _nonceData$pubNonce.x : undefined;
    ...
    if (keyResult && (nonceResult || extendedVerifierId || networkUtils.isLegacyNetwork(network)) || errorResult) {
      if (keyResult) {
      ...
      const serverTimeOffset = keyResult ? metadataHelpers.calculateMedian(serverTimeOffsets) : 0;
...
const VerifierLookupRequest = async params => {
  ...
  const result = await some.Some(lookupPromises, async lookupResults => {
    ...
    const errorResult = metadataHelpers.thresholdSame(lookupResults.map(x2 => x2 && x2.error), minThreshold);
    const keyResult = metadataHelpers.thresholdSame(lookupPubKeys.map(x3 => x3 && common.normalizeLookUpResult(x3.result)), minThreshold);
    const serverTimeOffsets = [];
...
const commitmentRequest = async params => {
  ...
  const tokenCommitment = metadataHelpers.keccak256(metadataHelpers.utf8ToBytes(idToken));
    ...
    some.Some(promiseArr, resultArr => {
      ...
      if (finalImportedShares.length > 0) {
        ...
        if (overrideExistingKey && completedRequests.length === endpoints.length) {
          ...
          const existingPubKey = metadataHelpers.thresholdSame(nodeSigs.map(x => x && x.pub_key_x), halfThreshold);
          ...
          if (existingPubKey || !existingPubKey && completedRequests.length === endpoints.length) {
            const requiredNodeResult = completedRequests.find(resp => {
              var _resp$result;
              return ((_resp$result = resp.result) === null || _resp$result === void 0 ? void 0 : _resp$result.nodeindex) === requiredNodeIndex;
            });
            if (requiredNodeResult) {
              return Promise.resolve(resultArr);
            }
...
async function retrieveOrImportShare(params) {
  ...
  await citadelUtils.callAllowApi({
    buildEnv,
    verifier,
    ...
    source,
    recordId,
    oauthInitiated: citadelUtils.CitadelAllowParamsSetOrUnsetFlag.SET,
    oauthCompleted: citadelUtils.CitadelAllowParamsSetOrUnsetFlag.SET
  });
  // generate temporary private and public key that is used to secure receive shares
  const sessionAuthKey = eccrypto.generatePrivate();
  const pubKey = metadataHelpers.bytesToHex(eccrypto.getPublic(sessionAuthKey));
  ...
  if ((newImportedShares === null || newImportedShares === void 0 ? void 0 : newImportedShares.length) > 0) {
    ...
    const privateKey = keyType === constants.KEY_TYPE.SECP256K1 ? metadataHelpers.generatePrivateKey(constants.KEY_TYPE.SECP256K1) : metadataHelpers.generatePrivateKey(constants.KEY_TYPE.ED25519);
    const generatedShares = await keyUtils.generateShares(ecCurve, keyType, serverTimeOffset, indexes, nodePubkeys, privateKey);
  ...
  if (checkCommitment) {
    ...
    const validCommitments = commitmentRequestResult.filter(x => !!x && typeof x === "object" && !x.error);
    for (const x of validCommitments) {
      nodeSigs.push(x.result);
    }
    // if user's account already
    isExistingKey = !!metadataHelpers.thresholdSame(nodeSigs.map(x => x && x.pub_key_x), halfThreshold);
    ...
    if (!overrideExistingKey) {
      ...
      const errorData = (_keyLookupResult$erro = keyLookupResult.errorResult) === null || _keyLookupResult$erro === void 0 ? void 0 : _keyLookupResult$erro.data;
  ...
  return some.Some(promiseArrRequest, async (shareResponseResult, sharedState) => {
    ...
    const thresholdPublicKey = metadataHelpers.thresholdSame(pubkeys, halfThreshold);
    ...
    shareResponses.forEach(x => {
      ...
      if (requiredShareResponse && !thresholdNonceData && !verifierParams.extended_verifier_id) {
        var _nonceData$pubNonce2;
        const currentPubKey = requiredShareResponse.public_key;
        const nonceData = requiredShareResponse.nonce_data;
        const pubNonce = nonceData && keyUtils.isV2NonceResult(nonceData) ? (_nonceData$pubNonce2 = nonceData.pubNonce) === null || _nonceData$pubNonce2 === void 0 ? void 0 : _nonceData$pubNonce2.x : undefined;
    ...
    if (completedRequests.length >= thresholdReqCount && thresholdPublicKey) {
      ...
      for (let i = 0; i < completedRequests.length; i += 1) {
        ...
        if ((sessionTokenSigs === null || sessionTokenSigs === void 0 ? void 0 : sessionTokenSigs.length) > 0) {
          ...
          if (sessionTokenSigMetadata && (_sessionTokenSigMetad = sessionTokenSigMetadata[0]) !== null && _sessionTokenSigMetad !== void 0 && _sessionTokenSigMetad.ephemPublicKey) {
            ...
            sessionTokenSigPromises.push(Promise.resolve(metadataHelpers.hexToBytes(sessionTokenSigs[0])));
          }
        } else {
        ...
        if ((sessionTokens === null || sessionTokens === void 0 ? void 0 : sessionTokens.length) > 0) {
          ...
          if (sessionTokenMetadata && (_sessionTokenMetadata = sessionTokenMetadata[0]) !== null && _sessionTokenMetadata !== void 0 && _sessionTokenMetadata.ephemPublicKey) {
            ...
            sessionTokenPromises.push(Promise.resolve(common.base64ToBytes(sessionTokens[0])));
          }
        } else {
        ...
        if ((keys === null || keys === void 0 ? void 0 : keys.length) > 0) {
          ...
          if (latestKey.share_metadata) {
            sharePromises.push(metadataUtils.decryptNodeDataWithPadding(latestKey.share_metadata, new TextDecoder().decode(common.base64ToBytes(latestKey.share)), sessionAuthKey).catch(err => loglevel.error("share decryption", err)));
          }
        } else {
      ...
      sessionTokensResolved.forEach((x, index) => {
        ...
        if (!x || !sig) sessionTokenData.push(undefined);else sessionTokenData.push({
          token: common.bytesToBase64(x),
          signature: metadataHelpers.bytesToHex(sig),
      ...
      const decryptedShares = sharesResolved.reduce((acc, curr, index) => {
        if (curr) {
          acc.push({
            ...
            value: metadataHelpers.bytesToNumberBE(curr)
          });
        }
        return acc;
      }, []);
      // run lagrange interpolation on all subsets, faster in the optimistic scenario than berlekamp-welch due to early exit
      const allCombis = metadataHelpers.kCombinations(decryptedShares.length, halfThreshold);
      let privateKey = null;
      for (let j = 0; j < allCombis.length; j += 1) {
        ...
        const derivedPrivateKey = metadataHelpers.lagrangeInterpolation(ecCurve, shares, indices);
        if (!derivedPrivateKey) continue;
        const decryptedPubKey = metadataHelpers.derivePubKey(ecCurve, derivedPrivateKey);
        const decryptedPubKeyX = decryptedPubKey.x;
        const decryptedPubKeyY = decryptedPubKey.y;
        if (decryptedPubKeyX === metadataHelpers.toBigIntBE(thresholdPublicKey.X) && decryptedPubKeyY === metadataHelpers.toBigIntBE(thresholdPublicKey.Y)) {
      ...
      return {
        ...
        isNewKey,
        serverTimeOffsetResponse: serverTimeOffset || metadataHelpers.calculateMedian(serverOffsetTimes)
      };
    }
    ...
    const oAuthPubKey = metadataHelpers.derivePubKey(ecCurve, oAuthKey);
    const oAuthPubkeyX = metadataHelpers.bigintToHex(oAuthPubKey.x);
    const oAuthPubkeyY = metadataHelpers.bigintToHex(oAuthPubKey.y);
    // if both thresholdNonceData and extended_verifier_id are not available
    // then we need to throw other wise address would be incorrect.
    if (!nonceResult && !verifierParams.extended_verifier_id && !networkUtils.isLegacyNetwork(network)) {
    ...
    let metadataNonce = (_nonceResult = nonceResult) !== null && _nonceResult !== void 0 && _nonceResult.nonce ? metadataHelpers.toBigIntBE(nonceResult.nonce) : 0n;
    let finalPubKey;
    let pubNonce;
    let typeOfUser = "v1";
    ...
    } else if (networkUtils.isLegacyNetwork(network)) {
      if (enableOneKey) {
        ...
        metadataNonce = nonceResult.nonce ? metadataHelpers.toBigIntBE(nonceResult.nonce) : 0n;
        typeOfUser = nonceResult.typeOfUser;
        if (typeOfUser === "v2" && keyUtils.isV2NonceResult(nonceResult)) {
          pubNonce = {
          ...
          const privateKeyWithNonce = metadataHelpers.mod(oAuthKey + metadataNonce, N);
          finalPubKey = metadataHelpers.derivePubKey(ecCurve, privateKeyWithNonce);
        }
      } else {
        typeOfUser = "v1";
        // for imported keys in legacy networks
    ...
    const oAuthKeyAddress = metadataHelpers.generateAddressFromPrivKey(keyType, oAuthKey);
    ...
    const finalWalletAddress = metadataHelpers.generateAddressFromPubKey(keyType, finalPubKey.x, finalPubKey.y);
    let keyWithNonce = "";
    if (typeOfUser === "v1") {
      isUpgraded = null;
    ...
    if (typeOfUser === "v1" || typeOfUser === "v2" && metadataNonce > 0n) {
      const privateKeyWithNonce = metadataHelpers.mod(oAuthKey + metadataNonce, N);
      keyWithNonce = metadataHelpers.bigintToHex(privateKeyWithNonce);
    }
    if (keyType === constants.KEY_TYPE.SECP256K1) {
      ...
      if (keyWithNonce && !nonceResult.seed) {
        ...
        const decryptedSeed = await metadataUtils.decryptSeedData(nonceResult.seed, metadataHelpers.toBigIntBE(keyWithNonce));
        finalPrivKey = metadataHelpers.bytesToHex(decryptedSeed);
      }
    } else {
    ...
    if (keyType === constants.KEY_TYPE.ED25519) {
      const {
        scalar,
        point
      } = metadataHelpers.getSecpKeyFromEd25519(privateKey);
      postboxKey = scalar;
      postboxPubX = metadataHelpers.bigintToHex(point.x);
      postboxPubY = metadataHelpers.bigintToHex(point.y);
    ...
    return {
      finalKeyData: {
        ...
        X: metadataHelpers.bigintToHex(finalPubKey.x),
        // this is final pub x user before and after updating to 2/n
        Y: metadataHelpers.bigintToHex(finalPubKey.y),
      ...
      oAuthKeyData: {
        ...
        X: oAuthPubkeyX,
        Y: oAuthPubkeyY,
        privKey: metadataHelpers.bigintToHex(oAuthKey)
      },
      postboxKeyData: {
        privKey: metadataHelpers.bigintToHex(postboxKey),
        X: postboxPubX,
        Y: postboxPubY
      },
      sessionData: {
        sessionTokenData,
        sessionAuthKey: metadataHelpers.bytesToHex(sessionAuthKey)
      },
      metadata: {
        pubNonce,
        nonce: metadataNonce,
