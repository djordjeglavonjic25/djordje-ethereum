...
function getTheme(theme) {
  if (theme === interfaces.THEME_MODES.light) return "light";
  if (theme === interfaces.THEME_MODES.dark) return "dark";
...
class AuthProvider {
  constructor({
    ...
    _defineProperty(this, "embedNonce", customauth.randomId());
    this.sdkUrl = sdkUrl;
  ...
  async init({
    ...
    const colorScheme = getTheme(this.whiteLabel.mode || interfaces.THEME_MODES.light);
    const authServiceIframe = utils.htmlToElement(`<iframe
        id="${constants.IFRAME_MODAL_ID}-${this.embedNonce}"
        class="${constants.IFRAME_MODAL_ID}-${this.embedNonce}"
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.messageHandler = event => {
          ...
          switch (type) {
            case constants.JRPC_METHODS.SETUP_COMPLETE:
              (_this$getAuthServiceI = this.getAuthServiceIframe()) === null || _this$getAuthServiceI === void 0 || (_this$getAuthServiceI = _this$getAuthServiceI.contentWindow) === null || _this$getAuthServiceI === void 0 || _this$getAuthServiceI.postMessage({
                type: constants.JRPC_METHODS.INIT_DAPP,
                data: {
                  network,
                  clientId
                }
              ...
              resolve();
              break;
            case constants.JRPC_METHODS.LOGIN_FAILED:
              (_this$loginCallbackFa = this.loginCallbackFailed) === null || _this$loginCallbackFa === void 0 || _this$loginCallbackFa.call(this, utils$1.isAuthFlowError(messageData) ? messageData.error : "Login failed, reason: unknown");
              break;
            case constants.JRPC_METHODS.DISPLAY_IFRAME:
              this.getAuthServiceIframe().style.display = "block";
              break;
            case constants.JRPC_METHODS.HIDE_IFRAME:
              this.getAuthServiceIframe().style.display = "none";
              break;
            case constants.JRPC_METHODS.LOGIN_SUCCESS:
              logger.log.info("LOGIN_SUCCESS", messageData);
              this.getAuthServiceIframe().style.display = "none";
              if (messageData && !utils$1.isAuthFlowError(messageData)) (_this$loginCallbackSu = this.loginCallbackSuccess) === null || _this$loginCallbackSu === void 0 || _this$loginCallbackSu.call(this, messageData);
              break;
            default:
              logger.log.warn(`Unknown message type: ${type}`);
              break;
          }
        };
  ...
  postLoginInitiatedMessage(loginConfig, nonce) {
    ...
    (_this$getAuthServiceI2 = this.getAuthServiceIframe().contentWindow) === null || _this$getAuthServiceI2 === void 0 || _this$getAuthServiceI2.postMessage({
      type: constants.JRPC_METHODS.LOGIN_INITIATED,
      data: {
        loginConfig,
        nonce
      }
  ...
  postLoginCancelledMessage(nonce) {
    ...
    (_this$getAuthServiceI3 = this.getAuthServiceIframe().contentWindow) === null || _this$getAuthServiceI3 === void 0 || _this$getAuthServiceI3.postMessage({
      type: constants.JRPC_METHODS.LOGIN_CANCELLED,
      data: {
        nonce
      }
    }, this.targetOrigin);
