...
import { microSeconds as microSeconds$1, generateRandomId, log, sleep } from '../util.js';
...
function postMessage(channelState, messageJson) {
  return new Promise((resolve, reject) => {
    sleep().then(async () => {
      ...
      const channelEncPrivKey = keccak256Bytes(utf8ToBytes(key));
      const encData = await encryptData(channelEncPrivKey, {
      ...
      const body = {
        ...
        data: encData,
        signature: bytesToHex(await sign(channelEncPrivKey, keccak256Bytes(utf8ToBytes(encData))))
      };
...
function setupSocketConnection(socketUrl, channelState, fn) {
  ...
  const listener = async ev => {
    try {
      const decData = await decryptData(channelEncPrivKey, ev);
      log.info(decData);
      fn(decData);
    } catch (error) {
      log.error(error);
