...
// 0.001 gwei = 1_000_000 wei. Rollups like Arbitrum Sepolia frequently report
// zero priority tips from both eth_feeHistory and eth_maxPriorityFeePerGas, so
// we need a non-zero floor to avoid submitting transactions with no headroom.
const MIN_PRIORITY_FEE_PER_GAS_WEI = new bignumber_js.BigNumber(1000000);
async function fetchGasEstimatesViaEthFeeHistory(provider) {
  ...
  let numberOfRewards;
  // Default to a single base-fee worth of headroom; doubled below when the
  // chain reports a zero priority tip from both feeHistory and the RPC.
  let baseFeeMultiplier = new bignumber_js.BigNumber(1);
  if (rewardList.length > 0) {
    ...
    let maxPriorityFeePerGas = new bignumber_js.BigNumber(maxPriorityFeePerGasResult, 16);
    if (maxPriorityFeePerGas.isZero()) {
      // Both feeHistory rewards and the RPC tip are zero: clamp to the floor
      // and pad maxFeePerGas with extra base-fee headroom to survive base-fee
      // spikes between estimation and inclusion.
      baseFeeMultiplier = new bignumber_js.BigNumber(2);
    }
    if (maxPriorityFeePerGas.lt(MIN_PRIORITY_FEE_PER_GAS_WEI)) {
      maxPriorityFeePerGas = MIN_PRIORITY_FEE_PER_GAS_WEI;
    }
    priorityFeeCalcs = {
  ...
  const baseFeeWithHeadroom = new bignumber_js.BigNumber(finalBaseFeePerGas).multipliedBy(baseFeeMultiplier);
  return {
    estimatedBaseFee: conversionUtils.hexWEIToDecGWEI(finalBaseFeePerGas).toString(10),
    high: {
      maxWaitTimeEstimate: 30000,
      minWaitTimeEstimate: 15000,
      suggestedMaxFeePerGas: conversionUtils.hexWEIToDecGWEI(priorityFeeCalcs.fast.div(numberOfRewards).plus(baseFeeWithHeadroom).toString(16)).toString(),
      suggestedMaxPriorityFeePerGas: conversionUtils.hexWEIToDecGWEI(priorityFeeCalcs.fast.div(numberOfRewards).toString(16)).toString()
    },
    medium: {
      maxWaitTimeEstimate: 45000,
      minWaitTimeEstimate: 15000,
      suggestedMaxFeePerGas: conversionUtils.hexWEIToDecGWEI(priorityFeeCalcs.average.div(numberOfRewards).plus(baseFeeWithHeadroom).toString(16)).toString(),
      suggestedMaxPriorityFeePerGas: conversionUtils.hexWEIToDecGWEI(priorityFeeCalcs.average.div(numberOfRewards).toString(16)).toString()
    },
    low: {
      maxWaitTimeEstimate: 60000,
      minWaitTimeEstimate: 15000,
      suggestedMaxFeePerGas: conversionUtils.hexWEIToDecGWEI(priorityFeeCalcs.slow.div(numberOfRewards).plus(baseFeeWithHeadroom).toString(16)).toString(),
      suggestedMaxPriorityFeePerGas: conversionUtils.hexWEIToDecGWEI(priorityFeeCalcs.slow.div(numberOfRewards).toString(16)).toString()
    }
  };
}
/**
