'use strict';

var _objectSpread = require('@babel/runtime/helpers/objectSpread2');
var _defineProperty = require('@babel/runtime/helpers/defineProperty');
var baseControllers = require('@toruslabs/base-controllers');
var metadataHelpers = require('@toruslabs/metadata-helpers');
var auth = require('@web3auth/auth');
var bignumber_js = require('bignumber.js');
var log = require('loglevel');
var viem = require('viem');
var eip7702Utils = require('../Eip7702/eip7702Utils.js');
var constants = require('../utils/constants.js');
var walletSendCalls = require('../Eip5792/walletSendCalls.js');
var conversionUtils = require('../utils/conversionUtils.js');
var helpers = require('../utils/helpers.js');
var interfaces = require('../utils/interfaces.js');
var NonceTracker = require('./NonceTracker.js');
var PendingTransactionTracker = require('./PendingTransactionTracker.js');
var TransactionGasUtil = require('./TransactionGasUtil.js');
var TransactionStateManager = require('./TransactionStateManager.js');
var TransactionUtils = require('./TransactionUtils.js');

class TransactionController extends TransactionStateManager.TransactionStateManager {
  constructor({
    config,
    state,
    provider,
    blockTracker,
    signEthTx,
    signEip7702Authorization,
    getCurrentChainId,
    getCurrentNetworkEIP1559Compatibility,
    getCurrentAccountEIP1559Compatibility,
    getSelectedAddress,
    getEIP1559GasFeeEstimates
  }) {
    super({
      config,
      state,
      getCurrentChainId
    });
    _defineProperty(this, "getSelectedAddress", void 0);
    _defineProperty(this, "getEIP1559GasFeeEstimates", void 0);
    _defineProperty(this, "nonceTracker", void 0);
    _defineProperty(this, "pendingTxTracker", void 0);
    _defineProperty(this, "txGasUtil", void 0);
    _defineProperty(this, "_getCurrentNetworkEIP1559Compatibility", void 0);
    _defineProperty(this, "_getCurrentAccountEIP1559Compatibility", void 0);
    _defineProperty(this, "signEthTx", void 0);
    _defineProperty(this, "signEip7702Authorization", void 0);
    _defineProperty(this, "provider", void 0);
    _defineProperty(this, "blockTracker", void 0);
    _defineProperty(this, "inProcessOfSigning", new Set());
    _defineProperty(this, "getUnapprovedTxCount", () => Object.keys(this.getUnapprovedTxList()).length);
    _defineProperty(this, "getPendingTxCount", account => this.getPendingTransactions(account).length);
    this.blockTracker = blockTracker;
    this._getCurrentNetworkEIP1559Compatibility = getCurrentNetworkEIP1559Compatibility;
    this._getCurrentAccountEIP1559Compatibility = getCurrentAccountEIP1559Compatibility;
    this.getSelectedAddress = getSelectedAddress;
    this.getEIP1559GasFeeEstimates = getEIP1559GasFeeEstimates;
    this.signEthTx = signEthTx;
    this.signEip7702Authorization = signEip7702Authorization;
    this.provider = provider;
    this.txGasUtil = new TransactionGasUtil.TransactionGasUtil(this.provider, this.blockTracker);
    this.nonceTracker = new NonceTracker.NonceTracker({
      provider,
      blockTracker,
      getConfirmedTransactions: this.getConfirmedTransactions.bind(this),
      getPendingTransactions: this.getSubmittedTransactions.bind(this) // nonce tracker should only care about submitted transactions
    });
    this.pendingTxTracker = new PendingTransactionTracker.PendingTransactionTracker({
      provider,
      nonceTracker: this.nonceTracker,
      getPendingTransactions: this.getPendingTransactions.bind(this),
      // pending tx tracker should only care about submitted and approved transactions
      getConfirmedTransactions: this.getConfirmedTransactions.bind(this),
      approveTransaction: this.approveTransaction.bind(this),
      publishTransaction: rawTx => this.provider.request({
        method: constants.METHOD_TYPES.ETH_SEND_RAW_TRANSACTION,
        params: [rawTx]
      })
    });
    this._setupListeners();
  }
  addTransactionUnapproved(txMeta) {
    this.addTransactionToState(txMeta);
    this.emit(`${txMeta.id}:unapproved`, txMeta);
  }
  async addNewUnapprovedTransaction(txParams, req) {
    const txMeta = await this.createTransaction(txParams, req);
    return this.processApproval(txMeta);
  }
  async addNewUnapprovedTransactionsBatch(batchRequest, req) {
    var _batchRequest$batchId;
    const batchRequestParams = Array.isArray(req.params) ? req.params[0] : req.params;
    const {
      from,
      chainId
    } = batchRequestParams;
    const {
      transactions,
      requiredEip7702Upgrade,
      eip7702UpgradeContractAddress: delegationAddress
    } = batchRequest;
    const batchId = (_batchRequest$batchId = batchRequest.batchId) !== null && _batchRequest$batchId !== void 0 ? _batchRequest$batchId : walletSendCalls.generateBatchId();
    let txParams;
    const txMetaOverride = {
      batchId,
      type: constants.CONTRACT_TYPE_ERC7821
    };
    if (transactions.length === 1) {
      // if it's single call, we can just process as a normal transaction
      txParams = {
        from,
        to: transactions[0].params.to,
        value: transactions[0].params.value,
        data: transactions[0].params.data,
        chainId
      };
    } else {
      // process multiple transactions as a batch
      const nestedTxMeta = await Promise.all(transactions.map(txParam => TransactionUtils.createNestedTransactionMeta(from, txParam, this.provider)));
      const batchTransaction = eip7702Utils.generateEIP7702BatchTransaction(from, nestedTxMeta);
      txParams = _objectSpread(_objectSpread({}, batchTransaction), {}, {
        from
      });
      if (requiredEip7702Upgrade) {
        if (!delegationAddress) {
          throw auth.rpcErrors.invalidParams("Delegation address is required for EIP-7702 upgrade");
        }
        txParams.authorizationList = [{
          address: delegationAddress,
          chainId: metadataHelpers.add0x(chainId)
        }];
        txParams.type = constants.TRANSACTION_ENVELOPE_TYPES.SET_CODE;
      }
      txMetaOverride.nestedTransactions = nestedTxMeta;
      txMetaOverride.transactionCategory = interfaces.TRANSACTION_CATEGORY_EIP7702.BATCH;
    }
    const txMeta = await this.createTransaction(txParams, req, txMetaOverride);
    await this.processApproval(txMeta);
    return batchId;
  }
  async processApproval(txMeta) {
    return new Promise((resolve, reject) => {
      const handleFinished = msg => {
        if (msg.status === baseControllers.TransactionStatus.rejected) {
          return reject(auth.providerErrors.userRejectedRequest(`Transaction Signature: User denied message signature`));
        }
        if (msg.status === baseControllers.TransactionStatus.failed) {
          return reject(auth.rpcErrors.internal(`Transaction Signature: failed to sign message ${msg.error}`));
        }
        if (msg.status === baseControllers.TransactionStatus.signed && msg.isSignOnly) {
          return resolve(msg.rawTransaction);
        }
        if (msg.status === baseControllers.TransactionStatus.submitted) {
          return resolve(msg.transactionHash);
        }
        return reject(auth.rpcErrors.internal(`Transaction Signature: Unknown problem: ${JSON.stringify(txMeta.transaction)}`));
      };
      this.once(`${txMeta.id}:finished`, handleFinished);
    });
  }
  async approveTransaction(transactionID) {
    const txMeta = this.getTransaction(transactionID);
    if (this.inProcessOfSigning.has(transactionID)) {
      return;
    }
    this.inProcessOfSigning.add(transactionID);
    let nonceLock;
    try {
      this.setTxStatusApproved(transactionID);
      const fromAddress = txMeta.transaction.from;
      const {
        customNonceValue
      } = txMeta.transaction;
      const customNonceValueNumber = Number(customNonceValue);
      nonceLock = await this.nonceTracker.getNonceLock(fromAddress);
      // add nonce to txParams
      // if txMeta has previousGasParams then it is a retry at same nonce with
      // higher gas settings and therefor the nonce should not be recalculated
      const nonce = nonceLock.nextNonce;
      const customOrNonce = customNonceValueNumber === 0 ? customNonceValue : customNonceValue || nonce;
      txMeta.transaction.nonce = metadataHelpers.add0x(customOrNonce.toString(16));
      // add nonce debugging information to txMeta
      txMeta.nonceDetails = nonceLock.nonceDetails;
      this.updateTransactionInState(txMeta, "transactions#approveTransaction");
      // sign transaction
      await this.signTransaction(transactionID, txMeta.isSignOnly);
      // publish if not signOnly
      if (!txMeta.isSignOnly) {
        await this.publishTransaction(transactionID);
      }
      nonceLock.releaseLock();
    } catch (err) {
      try {
        this.setTxStatusFailed(transactionID, err);
      } catch (err2) {
        log.error(err2);
      }
      // must set transaction to submitted/failed before releasing lock
      if (nonceLock) {
        nonceLock.releaseLock();
      }
      // continue with error chain
      throw err;
    } finally {
      this.inProcessOfSigning.delete(transactionID);
    }
  }
  async signTransaction(txId, isSignOnly) {
    const txMeta = this.getTransaction(txId);
    const chainId = this.getCurrentChainId();
    const type = this.getTransactionType(txMeta);
    const txParams = {
      type: Number.parseInt(type, 16),
      chainId,
      gasLimit: txMeta.transaction.gas,
      accessList: txMeta.transaction.accessList,
      data: txMeta.transaction.data,
      gasPrice: txMeta.transaction.gasPrice,
      maxFeePerGas: txMeta.transaction.maxFeePerGas,
      maxPriorityFeePerGas: txMeta.transaction.maxPriorityFeePerGas,
      nonce: Number.parseInt(txMeta.transaction.nonce, 16),
      to: txMeta.transaction.to,
      value: txMeta.transaction.value
    };
    const fromAddress = txMeta.transaction.from;
    // Sign Authorization if present
    if (txMeta.transaction.authorizationList) {
      const signedAuthorizations = [];
      txParams.authorizationList = [];
      for (const authorization of txMeta.transaction.authorizationList) {
        // prepare and normalize the authorization
        const authorizationNonce = authorization.nonce ? parseInt(authorization.nonce, 16) : parseInt(txMeta.transaction.nonce, 16) + 1;
        const normalizedAuthorization = {
          address: authorization.address,
          chainId: authorization.chainId,
          nonce: viem.toHex(authorizationNonce)
        };
        const signedAuthorization = await this.signEip7702Authorization(normalizedAuthorization, fromAddress);
        signedAuthorizations.push(signedAuthorization);
        txParams.authorizationList.push({
          address: signedAuthorization.address,
          chainId: BigInt(signedAuthorization.chainId),
          nonce: BigInt(signedAuthorization.nonce),
          signature: {
            r: signedAuthorization.r,
            s: signedAuthorization.s,
            yParity: parseInt(signedAuthorization.yParity, 16)
          }
        });
      }
      txMeta.transaction.authorizationList = signedAuthorizations;
    }
    const signedEthTx = await this.signEthTx(txParams, fromAddress);
    txMeta.r = signedEthTx.signature.r;
    txMeta.s = signedEthTx.signature.s;
    txMeta.v = signedEthTx.signature.v != null ? viem.toHex(signedEthTx.signature.v) : undefined;
    const rawTx = signedEthTx.serialized;
    txMeta.rawTransaction = rawTx;
    this.updateTransactionInState(txMeta, "transactions#signTransaction: add r, s, v values and rawTx");
    this.setTxStatusSigned(txId, isSignOnly);
    return rawTx;
  }
  async publishTransaction(txId) {
    const txMeta = this.getTransaction(txId);
    const rawTx = txMeta.rawTransaction;
    let txHash;
    try {
      txHash = await this.provider.request({
        method: constants.METHOD_TYPES.ETH_SEND_RAW_TRANSACTION,
        params: [rawTx]
      });
    } catch (error) {
      if (error.message.toLowerCase().includes("known transaction")) {
        txHash = viem.keccak256(metadataHelpers.add0x(rawTx));
      } else {
        throw error;
      }
    }
    this.setTxHash(txId, txHash);
    this.setTxStatusSubmitted(txId);
  }
  async confirmTransaction(params) {
    const {
      txId,
      txReceipt
    } = params;
    log.info(params, "confirm params");
    const txMeta = this.getTransaction(txId);
    if (!txMeta) return;
    try {
      txMeta.txReceipt = _objectSpread({}, txReceipt);
      this.setTxStatusConfirmed(txId);
      this.markNonceDuplicatesDropped(txId);
      this.updateTransactionInState(txMeta, "transactions#confirmTransaction - add txReceipt");
    } catch (error) {
      log.error(error);
    }
  }
  cancelTransaction(transactionID) {
    throw new Error(`Method not implemented. ${transactionID}`);
  }
  async getEIP1559Compatibility(fromAddress) {
    const currentNetworkIsCompatible = await this._getCurrentNetworkEIP1559Compatibility();
    const fromAccountIsCompatible = await this._getCurrentAccountEIP1559Compatibility(fromAddress);
    return currentNetworkIsCompatible && fromAccountIsCompatible;
  }
  async addTransactionGasDefaults(txMeta) {
    // for user operation, if the dapp doesn't provide gas and fee, let bundler estimate them
    // bundlers unfortunately have strict rules on fee prechecks – we will need to set a generous buffer
    if (txMeta.isUserOperation) return txMeta;
    let updateTxMeta = txMeta;
    try {
      updateTxMeta = await this.addTxGasDefaults(txMeta);
    } catch (error) {
      log.warn(error);
      updateTxMeta = this.getTransaction(txMeta.id);
      updateTxMeta.loadingDefaults = false;
      this.updateTransactionInState(txMeta, "Failed to calculate gas defaults.");
      throw error;
    }
    updateTxMeta.loadingDefaults = false;
    this.updateTransactionInState(updateTxMeta, "Added new unapproved transaction.");
    return updateTxMeta;
  }
  async addTxGasDefaults(txMeta) {
    const isSetCodeTx = TransactionUtils.isEip7702SetCodeTx(txMeta);
    // EIP-7702 type 4 (SET_CODE) transactions are EIP-1559 compatible
    // They use maxFeePerGas and maxPriorityFeePerGas like type 2 transactions
    const eip1559Compatibility = isSetCodeTx || txMeta.transaction.type !== constants.TRANSACTION_ENVELOPE_TYPES.LEGACY && (await this.getEIP1559Compatibility());
    const {
      gasPrice: defaultGasPrice,
      maxFeePerGas: defaultMaxFeePerGas,
      maxPriorityFeePerGas: defaultMaxPriorityFeePerGas
    } = await this.getDefaultGasFees(txMeta, eip1559Compatibility);
    const {
      gasLimit: defaultGasLimit,
      simulationFails
    } = await this.getDefaultGasLimit(txMeta);
    txMeta = this.getTransaction(txMeta.id);
    if (simulationFails) {
      txMeta.simulationFails = simulationFails;
    }
    this.applyFeeDefaults(txMeta.transaction, {
      gasPrice: defaultGasPrice,
      maxFeePerGas: defaultMaxFeePerGas,
      maxPriorityFeePerGas: defaultMaxPriorityFeePerGas
    }, eip1559Compatibility);
    if (defaultGasLimit && !txMeta.transaction.gas) {
      txMeta.transaction.gas = defaultGasLimit;
    }
    return txMeta;
  }
  /**
   * Build a fully populated unsigned transaction without writing to controller
   * state, signing, or publishing. The nonce lock is released before this
   * method returns, so callers issuing multiple prepare calls without
   * submission may receive duplicate nonces.
   */
  async prepareTransaction(txParams, opts = {}) {
    var _normalizedTxParamete, _txMeta$transaction$t;
    if (txParams.isUserOperation) {
      throw new Error("prepareTransaction does not support user operations; use AccountAbstractionController.prepareUserOp");
    }
    const {
      speed = interfaces.GasSpeed.MEDIUM,
      fillNonce = true
    } = opts;
    const normalizedTxParameters = TransactionUtils.normalizeTxParameters(txParams);
    const isSetCodeTx = normalizedTxParameters.type === constants.TRANSACTION_ENVELOPE_TYPES.SET_CODE && normalizedTxParameters.to === normalizedTxParameters.from && Boolean((_normalizedTxParamete = normalizedTxParameters.authorizationList) === null || _normalizedTxParamete === void 0 ? void 0 : _normalizedTxParamete.length);
    const eip1559Compatibility = isSetCodeTx || normalizedTxParameters.type !== constants.TRANSACTION_ENVELOPE_TYPES.LEGACY && (await this.getEIP1559Compatibility(normalizedTxParameters.from));
    TransactionUtils.validateTxParameters(normalizedTxParameters, eip1559Compatibility);
    const txMeta = this.generateTxMeta({
      transaction: _objectSpread(_objectSpread({}, normalizedTxParameters), {}, {
        value: normalizedTxParameters.value ? metadataHelpers.add0x(normalizedTxParameters.value) : "0x0"
      }),
      origin: normalizedTxParameters.origin
    });
    const defaultGasFees = await this.getGasFeesForSpeed(txMeta, eip1559Compatibility, speed);
    this.applyFeeDefaults(txMeta.transaction, defaultGasFees, eip1559Compatibility);
    const {
      gasLimit
    } = await this.getDefaultGasLimit(txMeta);
    if (gasLimit && !txMeta.transaction.gas) {
      txMeta.transaction.gas = gasLimit;
    }
    if (fillNonce && !txMeta.transaction.nonce) {
      const nonceLock = await this.nonceTracker.getNonceLock(normalizedTxParameters.from);
      try {
        txMeta.transaction.nonce = metadataHelpers.add0x(nonceLock.nextNonce.toString(16));
      } finally {
        nonceLock.releaseLock();
      }
    }
    const {
      from,
      to,
      value,
      data,
      gas,
      nonce,
      maxFeePerGas,
      maxPriorityFeePerGas,
      gasPrice,
      accessList,
      authorizationList
    } = txMeta.transaction;
    const prepared = {
      from: from,
      to: to,
      value: value,
      data: data,
      gas: gas,
      nonce: nonce,
      maxFeePerGas: maxFeePerGas,
      maxPriorityFeePerGas: maxPriorityFeePerGas,
      gasPrice: gasPrice,
      accessList,
      authorizationList,
      chainId: this.getCurrentChainId(),
      type: (_txMeta$transaction$t = txMeta.transaction.type) !== null && _txMeta$transaction$t !== void 0 ? _txMeta$transaction$t : eip1559Compatibility ? constants.TRANSACTION_ENVELOPE_TYPES.FEE_MARKET : constants.TRANSACTION_ENVELOPE_TYPES.LEGACY
    };
    return prepared;
  }
  setTxHash(txId, txHash) {
    // Add the tx hash to the persisted meta-tx object
    const txMeta = this.getTransaction(txId);
    txMeta.transactionHash = txHash;
    this.updateTransactionInState(txMeta, "transactions#setTxHash");
  }
  async getDefaultGasFees(txMeta, eip1559Compatibility) {
    return this.getGasFeesForSpeed(txMeta, eip1559Compatibility, interfaces.GasSpeed.MEDIUM);
  }
  async getDefaultGasLimit(txMeta) {
    var _txMeta$nestedTransac;
    const chainId = this.getCurrentChainId();
    const customNetworkGasBuffer = constants.CHAIN_ID_TO_GAS_LIMIT_BUFFER_MAP[chainId];
    const chainType = helpers.getChainType(chainId);
    const isSetCodeTx = TransactionUtils.isEip7702SetCodeTx(txMeta);
    if (txMeta.transaction.gas) {
      return {};
    }
    // For EIP-7702 type 4 transactions (SET_CODE), we need to estimate gas including authorization costs
    // Simple send optimization doesn't apply to SET_CODE transactions as they involve code execution
    if (!isSetCodeTx && txMeta.transaction.to && txMeta.transactionCategory === baseControllers.TRANSACTION_TYPES.SENT_ETHER && chainType !== "custom" && !txMeta.transaction.data) {
      // This is a standard ether simple send, gas requirement is exactly 21k
      return {
        gasLimit: helpers.GAS_LIMITS.SIMPLE
      };
    }
    const {
      blockGasLimit,
      estimatedGasHex,
      simulationFails
    } = await this.txGasUtil.analyzeGasUsage(txMeta);
    // add additional gas buffer to our estimation for safety
    let gasLimit;
    if (TransactionUtils.isEip7702UpgradeWithDataToSelfTransaction(txMeta) && (_txMeta$nestedTransac = txMeta.nestedTransactions) !== null && _txMeta$nestedTransac !== void 0 && _txMeta$nestedTransac.length) {
      // Base multiplier for EIP-7702 upgrade with data to self transactions
      // Add additional buffer based on the number of batch calls in ERC7821 execute
      const batchCallsCount = txMeta.nestedTransactions.length;
      const baseMultiplier = 1.5;
      const perCallBuffer = 0.1;
      const eip7702UpgradeWithDataToSelfGasBufferMultiplier = baseMultiplier + perCallBuffer * batchCallsCount;
      gasLimit = this.txGasUtil.addGasBuffer(metadataHelpers.add0x(estimatedGasHex), blockGasLimit, eip7702UpgradeWithDataToSelfGasBufferMultiplier);
    } else {
      gasLimit = this.txGasUtil.addGasBuffer(metadataHelpers.add0x(estimatedGasHex), blockGasLimit, customNetworkGasBuffer);
    }
    return {
      gasLimit,
      simulationFails
    };
  }
  applyFeeDefaults(tx, defaults, eip1559Compatibility) {
    const {
      gasPrice: defaultGasPrice,
      maxFeePerGas: defaultMaxFeePerGas,
      maxPriorityFeePerGas: defaultMaxPriorityFeePerGas
    } = defaults;
    if (eip1559Compatibility) {
      // If the dapp has suggested a gas price, but no maxFeePerGas or maxPriorityFeePerGas
      //  then we set maxFeePerGas and maxPriorityFeePerGas to the suggested gasPrice.
      if (tx.gasPrice && !tx.maxFeePerGas && !tx.maxPriorityFeePerGas) {
        tx.maxFeePerGas = tx.gasPrice;
        // If the dapp has suggested a gas price, but no maxFeePerGas or maxPriorityFeePerGas
        // then we set maxFeePerGas to the suggested gasPrice.
        tx.maxPriorityFeePerGas = helpers.bnLessThan(typeof defaultMaxPriorityFeePerGas === "string" ? metadataHelpers.remove0x(defaultMaxPriorityFeePerGas) : defaultMaxPriorityFeePerGas, typeof tx.gasPrice === "string" ? metadataHelpers.remove0x(tx.gasPrice) : tx.gasPrice) ? defaultMaxPriorityFeePerGas : tx.gasPrice;
      } else {
        if (defaultMaxFeePerGas && !tx.maxFeePerGas) {
          // If the dapp has not set the gasPrice or the maxFeePerGas, then we set maxFeePerGas
          // with the one returned by the gasFeeController, if that is available.
          tx.maxFeePerGas = defaultMaxFeePerGas;
        }
        if (defaultMaxPriorityFeePerGas && !tx.maxPriorityFeePerGas) {
          // If the dapp has not set the gasPrice or the maxPriorityFeePerGas, then we set maxPriorityFeePerGas
          // with the one returned by the gasFeeController, if that is available.
          tx.maxPriorityFeePerGas = defaultMaxPriorityFeePerGas;
        }
        if (defaultGasPrice && !tx.maxFeePerGas) {
          // If the dapp has not set the gasPrice or the maxFeePerGas, and no maxFeePerGas is available
          // from the gasFeeController, then we set maxFeePerGas to the defaultGasPrice, assuming it is
          // available.
          tx.maxFeePerGas = defaultGasPrice;
        }
        if (tx.maxFeePerGas && !tx.maxPriorityFeePerGas) {
          // If the dapp has not set the gasPrice or the maxPriorityFeePerGas, and no maxPriorityFeePerGas is
          // available from the gasFeeController, then we set maxPriorityFeePerGas to
          // txMeta.transaction.maxFeePerGas, which will either be the gasPrice from the controller, the maxFeePerGas
          // set by the dapp, or the maxFeePerGas from the controller.
          tx.maxPriorityFeePerGas = tx.maxFeePerGas;
        }
      }
      // We remove the gasPrice param entirely when on an eip1559 compatible network
      delete tx.gasPrice;
    } else {
      // We ensure that maxFeePerGas and maxPriorityFeePerGas are not in the transaction params
      // when not on a EIP1559 compatible network
      delete tx.maxPriorityFeePerGas;
      delete tx.maxFeePerGas;
    }
    // If we have gotten to this point, and none of gasPrice, maxPriorityFeePerGas or maxFeePerGas are
    // set on transaction, it means that either we are on a non-EIP1559 network and the dapp didn't suggest
    // a gas price, or we are on an EIP1559 network, and none of gasPrice, maxPriorityFeePerGas or maxFeePerGas
    // were available from either the dapp or the network.
    if (defaultGasPrice && !tx.gasPrice && !tx.maxPriorityFeePerGas && !tx.maxFeePerGas) {
      tx.gasPrice = defaultGasPrice;
    }
  }
  async getGasFeesForSpeed(txMeta, eip1559Compatibility, speed = interfaces.GasSpeed.MEDIUM) {
    if (!eip1559Compatibility && txMeta.transaction.gasPrice || eip1559Compatibility && txMeta.transaction.maxFeePerGas && txMeta.transaction.maxPriorityFeePerGas) {
      return {};
    }
    try {
      const {
        gasFeeEstimates,
        gasEstimateType
      } = await this.getEIP1559GasFeeEstimates();
      if (eip1559Compatibility && gasEstimateType === constants.GAS_ESTIMATE_TYPES.FEE_MARKET) {
        // this is in dec gwei
        const {
          [speed]: {
            suggestedMaxPriorityFeePerGas,
            suggestedMaxFeePerGas
          } = {}
        } = gasFeeEstimates;
        if (suggestedMaxPriorityFeePerGas && suggestedMaxFeePerGas) {
          return {
            // send to controller in hex wei
            maxFeePerGas: metadataHelpers.add0x(conversionUtils.decGWEIToHexWEI(new bignumber_js.BigNumber(suggestedMaxFeePerGas)).toString(16)),
            maxPriorityFeePerGas: metadataHelpers.add0x(conversionUtils.decGWEIToHexWEI(new bignumber_js.BigNumber(suggestedMaxPriorityFeePerGas)).toString(16))
          };
        }
      } else if (gasEstimateType === constants.GAS_ESTIMATE_TYPES.LEGACY) {
        const {
          medium
        } = gasFeeEstimates;
        // Legacy chains keep the existing medium estimate regardless of speed.
        return {
          gasPrice: metadataHelpers.add0x(conversionUtils.decGWEIToHexWEI(new bignumber_js.BigNumber(medium)).toString(16))
        };
      } else if (gasEstimateType === constants.GAS_ESTIMATE_TYPES.ETH_GASPRICE) {
        const {
          gasPrice
        } = gasFeeEstimates;
        // The ETH_GASPRICE type just includes a single gas price property,
        // which we can assume was retrieved from eth_gasPrice
        return {
          gasPrice: metadataHelpers.add0x(conversionUtils.decGWEIToHexWEI(new bignumber_js.BigNumber(gasPrice)).toString(16))
        };
      }
    } catch (error) {
      log.error(error);
    }
    const gasPrice = await this.provider.request({
      method: constants.METHOD_TYPES.ETH_GET_GAS_PRICE
    });
    return {
      gasPrice: gasPrice && metadataHelpers.add0x(gasPrice)
    };
  }
  async createTransaction(txParameters, req, overrideTxMeta) {
    const normalizedTxParameters = TransactionUtils.normalizeTxParameters(txParameters);
    const eip1559Compatibility = await this.getEIP1559Compatibility(txParameters.from);
    TransactionUtils.validateTxParameters(normalizedTxParameters, eip1559Compatibility);
    let txMeta = this.generateTxMeta({
      transaction: normalizedTxParameters,
      origin: req.origin
    });
    const {
      type,
      category,
      methodParams
    } = await TransactionUtils.determineTransactionType(txParameters, this.provider, overrideTxMeta);
    txMeta.contractType = type;
    txMeta.transactionCategory = category;
    txMeta.methodParams = methodParams;
    txMeta.isUserOperation = req.isUserOperation;
    txMeta.isSignOnly = req.isSignOnly;
    txMeta.transaction.value = txMeta.transaction.value ? metadataHelpers.add0x(txMeta.transaction.value) : "0x0";
    txMeta.batchId = overrideTxMeta === null || overrideTxMeta === void 0 ? void 0 : overrideTxMeta.batchId;
    txMeta.nestedTransactions = overrideTxMeta === null || overrideTxMeta === void 0 ? void 0 : overrideTxMeta.nestedTransactions;
    this.emit(`${txMeta.id}:unapproved`, txMeta);
    txMeta = this.addTransactionToState(txMeta);
    txMeta = await this.addTransactionGasDefaults(txMeta);
    this.emit(baseControllers.TX_EVENTS.TX_UNAPPROVED, {
      txMeta,
      req
    });
    return txMeta;
  }
  _setupListeners() {
    this.setupBlockTrackerListener();
    this.pendingTxTracker.on(baseControllers.TX_EVENTS.TX_WARNING, data => {
      this.updateTransactionInState(data.txMeta);
    });
    this.pendingTxTracker.on(baseControllers.TX_EVENTS.TX_DROPPED, data => this.setTxStatusDropped(data.txId));
    this.pendingTxTracker.on(baseControllers.TX_EVENTS.TX_BLOCK_UPDATE, ({
      txMeta,
      latestBlockNumber
    }) => {
      if (!txMeta.firstRetryBlockNumber) {
        txMeta.firstRetryBlockNumber = latestBlockNumber;
        this.updateTransactionInState(txMeta);
      }
    });
    this.pendingTxTracker.on(baseControllers.TX_EVENTS.TX_RETRY, data => {
      if (!("retryCount" in data.txMeta)) {
        data.txMeta.retryCount = 0;
      }
      data.txMeta.retryCount += 1;
      this.updateTransactionInState(data.txMeta);
    });
    this.pendingTxTracker.on(baseControllers.TX_EVENTS.TX_FAILED, data => {
      this.setTxStatusFailed(data.txId, data.error);
    });
    this.pendingTxTracker.on(baseControllers.TX_EVENTS.TX_CONFIRMED, data => this.confirmTransaction(data));
  }
  setupBlockTrackerListener() {
    let listenersAreActive = false;
    const latestBlockHandler = this.onLatestBlock.bind(this);
    this.on(baseControllers.TX_EVENTS.TX_STATUS_UPDATE, () => {
      const pendingTxs = this.getPendingTransactions();
      if (!listenersAreActive && pendingTxs.length > 0) {
        this.blockTracker.on("latest", latestBlockHandler);
        listenersAreActive = true;
      } else if (listenersAreActive && !pendingTxs.length) {
        this.blockTracker.removeListener("latest", latestBlockHandler);
        listenersAreActive = false;
      }
    });
  }
  async onLatestBlock(blockNumber) {
    try {
      await this.pendingTxTracker.updatePendingTxs();
    } catch (error) {
      log.error(error);
    }
    try {
      await this.pendingTxTracker.resubmitPendingTxs(blockNumber);
    } catch (error) {
      log.error(error);
    }
  }
  markNonceDuplicatesDropped(txId) {
    const txMeta = this.getTransaction(txId);
    const {
      nonce,
      from
    } = txMeta.transaction;
    const sameNonceTxs = this.getTransactions({
      searchCriteria: {
        from,
        nonce
      }
    });
    if (!sameNonceTxs.length) return;
    sameNonceTxs.forEach(tx => {
      if (tx.id === txId) return;
      this.updateTransactionInState(txMeta, "transactions/pending-tx-tracker#event: tx:confirmed reference to confirmed txHash with same nonce");
      if (tx.status !== baseControllers.TransactionStatus.failed) this.setTxStatusDropped(tx.id);
    });
  }
  getTransactionType(txMeta) {
    if (TransactionUtils.isEip7702SetCodeTx(txMeta)) {
      return constants.TRANSACTION_ENVELOPE_TYPES.SET_CODE;
    }
    return TransactionUtils.isEIP1559Transaction(txMeta) ? constants.TRANSACTION_ENVELOPE_TYPES.FEE_MARKET : constants.TRANSACTION_ENVELOPE_TYPES.LEGACY;
  }
}

exports.TransactionController = TransactionController;
