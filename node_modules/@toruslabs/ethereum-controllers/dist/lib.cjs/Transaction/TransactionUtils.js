...
function ensureProperTransactionEnvelopeTypeProvided(txParams, field) {
  switch (field) {
    ...
    case "maxPriorityFeePerGas":
      if (txParams.type && txParams.type !== constants.TRANSACTION_ENVELOPE_TYPES.FEE_MARKET && txParams.type !== constants.TRANSACTION_ENVELOPE_TYPES.SET_CODE) {
    ...
    switch (key) {
      ...
      case "authorizationList":
        if (!Array.isArray(value)) {
          throw auth.rpcErrors.invalidParams(`Invalid transaction params: ${key} is not an array. got: (${value})`);
        }
...
function isEip7702SetCodeTx(txMeta) {
  return txMeta.transaction.type === constants.TRANSACTION_ENVELOPE_TYPES.SET_CODE && txMeta.transaction.to === txMeta.transaction.from && Boolean(txMeta.transaction.authorizationList && txMeta.transaction.authorizationList.length > 0);
}
/**
...
exports.createNestedTransactionMeta = createNestedTransactionMeta;
...
exports.isEip7702SetCodeTx = isEip7702SetCodeTx;
exports.isEip7702UpgradeWithDataToSelfTransaction = isEip7702UpgradeWithDataToSelfTransaction;
