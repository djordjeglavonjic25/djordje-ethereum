...
import { decodeFunctionData, decodeAbiParameters, parseAbiParameters, toHex, getAddress, zeroAddress, encodeAbiParameters, encodeFunctionData, toFunctionSelector } from 'viem';
...
const ERC7821_EXECUTE_SELECTOR = toFunctionSelector(erc7821Abi[0]);
function isAtomicBatchTransaction(txParams) {
  if (!txParams || !Array.isArray(txParams) || txParams.length !== 1) {
    return false;
  }
  const {
    data,
  ...
  if (!data || data === "0x" || !from || !to) {
    return false;
  }
  return from.toLowerCase() === to.toLowerCase() && data.toLowerCase().startsWith(ERC7821_EXECUTE_SELECTOR);
}

/**
 * Decode an EIP-7702 / ERC-7821 batch transaction into its mode and nested transactions.
 *
 * Inverse of {@link generateEIP7702BatchTransaction}.
 */
function decodeEIP7821BatchTransaction(batchTransaction) {
  const {
    data
  } = batchTransaction;
  if (!data) {
  ...
  if (decoded.functionName !== "execute") {
    throw new Error(`Expected ERC-7821 execute call, received ${decoded.functionName}`);
  }
  const [mode, executionData] = decoded.args;
  const decodedCalls = decodeAbiParameters(parseAbiParameters("(address target, uint256 value, bytes callData)[]"), executionData);
  const transactions = decodedCalls[0].map(call => ({
...
export { MetaMask_EIP7702_Stateless_Delegator, decodeEIP7821BatchTransaction, generateEIP7702BatchTransaction, getDelegationAddress, getIsEip7702UpgradeSupported, isAtomicBatchTransaction };
