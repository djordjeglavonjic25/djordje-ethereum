...
class MetamaskWallet {
    ...
    constructor({ client, walletName }) {
        ...
        _MetamaskWallet_disconnect.set(this, async (options = {}) => {
            ...
            if (revokeSession) {
                await this.client.revokeSession({ scopes: [types_1.Scope.MAINNET, types_1.Scope.DEVNET, types_1.Scope.TESTNET] });
            }
        });
...
async function _MetamaskWallet_handleAccountsChangedEvent(data) {
    ...
    if (!addressToSelect) {
        ...
        await __classPrivateFieldGet(this, _MetamaskWallet_disconnect, "f").call(this, { revokeSession: false });
        return;
    }
    const session = await this.client.getSession();
